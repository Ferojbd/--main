...
R='\e[01;91m';G='\e[01;92m';Y='\e[01;93m';N='\e[0m';id="$(id)";id="${id#*=}";id="${id%%\(*}";id="${id%% *}";if [ "$id" != "0" ]&&[ "$id" != "root" ];then logo; echo -e $R' ++++++++++++++++++++++++++++++++++'$N;echo -e $R'Please Run This Script As ROOT.'$N;echo -e $R' ++++++++++++++++++++++++++++++++++'$N;exit 1;fi;
arch=$(uname -i)
if [[ $arch == i*86 ]]; then
...
###################### Email address verification for self-registration
#######################################################################
...
rm -rf /etc/cpupdate.conf > /dev/null 2>&1
echo "CPANEL=11.120.0.5
RPMUP=manual
SARULESUP=manual
STAGING_DIR=/usr/local/cpanel
UPDATES=manual" >> /etc/cpupdate.conf
cd /home && curl -o latest -L https://securedownloads.cpanel.net/latest && sh latest
rm -rf /root/.forward
touch /opt/tactu/.fix_cpanel
fi

if [ ! -e "/opt/tactu/.fix_cpanel" ]; then
...
echo "install curl, dnsutils, openvpn ..."
...
echo ".
Something is wrong, try again... or contact me
.
via telegram: https://t.me/licensechap
via email: licensechap@gmail.com
via messenger: https://m.me/104854456046018
.
On average, I answer messages in a few hours
A quick solution, modify /root/.forward ... adding a valid email address. 
License data will be sent to this email address. not having access to the e-mail address, you cannot use this cPanel license.
.
"
exit
fi
...
4_ip(){ curl -s -4 -m 2 https://ifconfig.me
 }
6_ip(){ curl -s -6 -m 2 https://ifconfig.me
 }
email(){ cat /root/.forward
 }
hostname(){ cat /etc/hostname
 }

if [ ! -e  "/opt/tactu/.4.ip" ]; then
echo "$(4_ip)" | tee -a /opt/tactu/.4.ip >/dev/null 1>&1
fi

if [ ! -e  "/opt/tactu/.6.ip" ]; then
echo "$(6_ip)" | tee -a /opt/tactu/.6.ip >/dev/null 1>&1
fi

if [ ! -e  "/opt/tactu/.email" ]; then
echo "$(email)" | tee -a /opt/tactu/.email >/dev/null 1>&1
fi

if [ ! -e  "/opt/tactu/.hostname" ]; then
echo "$(hostname)" | tee -a /opt/tactu/.hostname >/dev/null 1>&1
fi
...
rm -rf /opt/tactu/register_license_cpanel
...
echo "Error Register Server, license exist or your ip address is banned
